...
export default function QuizSection({ questions, onRestart }: QuizSectionProps) {
  ...
  if (quizComplete) {
    ...
    return (
      <motion.div
        ...
        className="bg-white rounded-2xl p-8 shadow-xl border border-gray-100 text-center space-y-6"
      >
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-gray-900">أحسنت! لقد أكملت الاختبار</h2>
          <p className="text-gray-500">نتيجتك النهائية هي {score} من {questions.length}</p>
        </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100 space-y-6 max-w-2xl mx-auto">
      {/* Quiz Header */}
      <div className="flex items-center justify-between border-b pb-4">
        ...
        <div className="text-xs font-bold text-gray-400 bg-gray-50 px-3 py-1 rounded-full border">
      ...
      <div className="space-y-6">
        <h3 className="text-lg font-bold text-gray-800 leading-relaxed">{currentQuestion.question}</h3>
          ...
          {currentQuestion.type === 'short-answer' && (
            <div className="space-y-4">
              <input
                ...
                className="w-full p-4 rounded-xl border-2 border-gray-100 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all text-gray-800"
              />
              {!selectedAnswer && (
                <button
        ...
        <AnimatePresence>
          {showExplanation && (
            <motion.div
              ...
              className="bg-blue-50 p-4 rounded-xl border border-blue-100 space-y-2"
            >
              ...
              {currentQuestion.type === 'short-answer' && (
                <p className="text-xs text-blue-600 mt-2 italic">الإجابة الصحيحة كانت: {currentQuestion.correctAnswer}</p>
              )}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {selectedAnswer && (
          <button
            onClick={handleNext}
            className="w-full flex items-center justify-center gap-2 bg-gray-900 text-white p-4 rounded-xl font-bold hover:bg-black transition-all shadow-lg active:scale-95 mt-4"
          >
