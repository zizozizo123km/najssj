...
export default function ProfileHeader({ user, onEdit }: ProfileHeaderProps) {
  return (
    <motion.div
      ...
      className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-800 transition-colors"
    >
      <div className="flex flex-col md:flex-row items-center gap-6">
        <div className="relative group">
          <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-blue-50 dark:border-blue-900/30 shadow-inner">
            {user.photoURL ? (
        ...
        <div className="flex-1 text-center md:text-right space-y-2">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-3">
            {user.favoriteSubjects.map((subject) => (
              <span key={subject} className="flex items-center gap-1 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-3 py-1 rounded-full text-xs font-bold border border-blue-100 dark:border-blue-900/50">
