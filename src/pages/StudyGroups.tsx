...
import { auth, db, collection, query, orderBy, onSnapshot, doc, getDoc, writeBatch, setDoc, onAuthStateChanged } from '../lib/firebase';
...
export default function StudyGroups() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      ...
      unsubscribeProfile = onSnapshot(doc(db, 'profiles', user.uid), async (snapshot) => {
        if (snapshot.exists()) {
